...
// Load prompts and filter for missing items
const streetwearItems = require('./prompts/streetwear_items.json');
const MISSING_ITEMS = ['ls9', 'ls20', 'ls51', 'ls54'];
const ASSETS_TO_GENERATE = streetwearItems
    .filter(item => MISSING_ITEMS.includes(item.name))
    .map(asset => ({
        ...asset,
...
// Main function
async function run() {
    console.log(`🚀 Generating ${ASSETS_TO_GENERATE.length} missing Luxury Sneakers images...\n`);
